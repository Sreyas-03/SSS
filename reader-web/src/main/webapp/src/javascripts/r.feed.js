...
r.feed.context = {
  ...
  displayArticles: [] // Array to hold currently displayed articles
};
...
r.feed.init = function () {
  ...
  $.History.bind('/feed/', function (state, target) {
    ...
    r.feed.context.displayArticles = [];
...
r.feed.load = function (next) {
  ...
  r.feed.activeXhr = r.util.ajax({
    ...
    data: data,
      ...
      for (let key in r.subscription.checkedSubscriptions) {
        if (r.subscription.checkedSubscriptions[key] === true) {
          check = true;
          break;
        }
      }
      if (!check) {
        ...
        // Filter the articles based on checked subscriptions.
        const checkedSubscriptionsLength = Object.keys(r.subscription.checkedSubscriptions).length;

        if (checkedSubscriptionsLength > 0) { // Only filter if there are checked subscriptions
          displayArticles = data.articles.filter(article => {
            return r.subscription.checkedSubscriptions[article.subscription.title] === true;
          });
        }
        else {
          displayArticles = data.articles;
      ...
      var nbArticles = $(displayArticles).length;
      ...
      console.log("Display articles", displayArticles);

      // Building articles
      r.feed.context.displayArticles = [];
      $(displayArticles).each(function (i, article) {
        // Escape some fields
        ...
        r.feed.context.displayArticles.push(article.id); // Store displayed article id
